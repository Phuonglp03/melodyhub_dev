...
export const ensureConversationWith = async (peerId) => {
  ...
  const { data } = await api.post(`/dm/conversations/${peerId}`);
  return data?.data;
};
...
export const acceptConversation = async (conversationId) => {
  ...
  const { data } = await api.post(`/dm/conversations/${conversationId}/accept`);
  return data?.data;
};
...
export const declineConversation = async (conversationId) => {
  if (!conversationId) throw new Error('conversationId is required');
  const { data } = await api.post(`/dm/conversations/${conversationId}/decline`);
  return data;
};
...
export const sendMessage = async (conversationId, text) => {
  if (!conversationId) throw new Error('conversationId is required');
  const { data } = await api.post(`/dm/conversations/${conversationId}/messages`, { text });
  return data?.data;
};
...
export const markSeen = async (conversationId) => {
  if (!conversationId) throw new Error('conversationId is required');
  const { data } = await api.post(`/dm/conversations/${conversationId}/seen`);
  return data;
};
